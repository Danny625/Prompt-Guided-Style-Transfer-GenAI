...
def try_load_clip(device: torch.device, skip_clip: bool):
    if skip_clip:
        return None, None
    ...
    try:
        ...
        print(f"Skipping CLIP metrics because CLIP could not be loaded: {exc}")
        return None, None
...
def clip_scores(
    ...
    text_image_inputs = clip_processor(text=[prompt], images=[output], return_tensors="pt", padding=True).to(device)
    text_image_outputs = clip_model(**text_image_inputs)
...
def main() -> None:
    ...
    clip_model, clip_processor = try_load_clip(device=device, skip_clip=args.skip_clip)
    ...
    for index, row in enumerate(rows):
        ...
        prompt_alignment, content_similarity = clip_scores(
    ...
    summary = [
        {
            "method": "InstructPix2Pix baseline",
